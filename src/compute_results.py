...
def _ensure_task_dir(dataset_path, task):
    os.makedirs(f'{dataset_path}/task/{task}', exist_ok=True)

import datetime


def _clip_prefix_from_kwargs(kwargs) -> str:
...
def _should_save_outputs(kwargs) -> bool:
    return bool(kwargs.get("save_outputs", True))
...
def fiq(
    ...
    **kwargs
    ...
    if _should_save_outputs(kwargs):
        _ensure_task_dir(dataset_path, task)
        result_path = f'{dataset_path}/task/{task}/result_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_{ways}_loop_{loop}_{get_time()}.json'
...
def fiq_fuse2paths(
    ...
    **kwargs
    ...
    if _should_save_outputs(kwargs):
        _ensure_task_dir(dataset_path, task)
        recall_results_filename = f"{dataset_path}/task/{task}/top_rank_fused_{_clip_prefix_from_kwargs(kwargs)}loop_{loop}_{task}_{get_time()}.json"
        ...
        result_path = f"{dataset_path}/task/{task}/result_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_loop_{loop}_{get_time()}.json"
        with open(result_path, 'w') as f:
            json.dump(output_metrics, f, indent=6)
...
def cirr(
    ...
    **kwargs
    ...
    if split == 'test':
        ...
        if _should_save_outputs(kwargs):
            submissions_folder_path = f"{dataset_path}/task/{task}/test_submissions_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_{ways}_cirr_loop_{loop}_{get_time()}.json"
            group_submissions_folder_path = f"{dataset_path}/task/{task}/subset_test_submissions_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_{ways}_cirr_loop_{loop}_{get_time()}.json"
    ...
    if _should_save_outputs(kwargs):
        _ensure_task_dir(dataset_path, task)
        result_path = f'{dataset_path}/task/{task}/result_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_{ways}_cirr_loop_{loop}_{get_time()}.json'
        with open(result_path,'w') as f:
            json.dump(output_metrics, f, indent=6)
    ...
    **kwargs
    ...
    for i in range(Q):
        ...
        if split == 'test':
            ...
            sorted_group_names = [name for name in filtered_sorted_names if name in current_group_targets]
            group_recall_1 = 1 if current_target_name in sorted_group_names[:1] else 0
            group_recall_2 = 1 if current_target_name in sorted_group_names[:2] else 0
            group_recall_3 = 1 if current_target_name in sorted_group_names[:3] else 0
    ...
    if split == 'test':
        ...
        if _should_save_outputs(kwargs):
            _ensure_task_dir(dataset_path, task)
            submissions_folder_path = f"{dataset_path}/task/{task}/test_submissions_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_cirr_loop_{loop}_{get_time()}.json"
            group_submissions_folder_path = f"{dataset_path}/task/{task}/subset_test_submissions_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_cirr_loop_{loop}_{get_time()}.json"
...
def circo(
    ...
    **kwargs
    ...
    if split == 'test':
        ...
        if _should_save_outputs(kwargs):
            _ensure_task_dir(dataset_path, task)
            output_path = f"{dataset_path}/task/{task}/test_submissions_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_{ways}_loop_{loop}_{get_time()}.json"
            with open(output_path,'w') as f:
                json.dump(queryid_to_retrieved_images, f, indent=6)
...
def _normalize_circo_name_for_val(name: str) -> str:
    name = os.path.splitext(os.path.basename(str(name)))[0].strip()
    if name.isdigit():
        return str(int(name))
    name = name.lstrip("0")
...
def circo_fuse2paths(
    ...
    **kwargs
    ...
    """
    Compute the retrieval metrics on the CIRCO validation/test set using fused candidates from two paths.
    """
    topk = 50
    top2k = 2 * topk

    def _normalize_circo_key(name: str) -> str:
        """
        Normalize a CIRCO image identifier into a comparison key.
        Used only for matching candidate-path names back to canonical index_names.
        """
        name = os.path.splitext(os.path.basename(str(name)))[0].strip()
        if name.isdigit():
            return str(int(name))
    ...
    # Build a map from normalized key -> canonical gallery/index name
    # This is the critical fix for stage2 val becoming all-zero.
    canonical_name_map = {}
    for idx_name in index_names:
        key = _normalize_circo_key(idx_name)
        if key not in canonical_name_map:
            canonical_name_map[key] = str(idx_name)

    def _to_canonical_name_from_candidate(candidate_path_or_name: str) -> str:
        """
        Convert a candidate path/name into the exact canonical name used in index_names when possible.
        """
        raw_key = _normalize_circo_key(candidate_path_or_name)
        raw_base = os.path.splitext(os.path.basename(str(candidate_path_or_name)))[0].strip()
        return canonical_name_map.get(raw_key, raw_base)

    def _to_canonical_name_from_gt(name: str) -> str:
        """
        Convert a GT/reference/target name into the same canonical namespace.
        """
        key = _normalize_circo_key(name)
        return canonical_name_map.get(key, str(name))
    ...
    # --------------------------------------------------
    # Unified stage2 ranking for both val and test
    # --------------------------------------------------
    for i in range(Q):
        query_candidates1 = candidates1[i]
        query_candidates2 = candidates2[i]
        ...
        for candidate in query_candidates1:
            candidate_path = candidate[1]
            candidate_name = _to_canonical_name_from_candidate(candidate_path)
            confidence_map1[candidate_name] = candidate[2]
            candidate_set.add(candidate_name)
        ...
        for candidate in query_candidates2:
            candidate_path = candidate[1]
            candidate_name = _to_canonical_name_from_candidate(candidate_path)
            confidence_map2[candidate_name] = candidate[2]
            candidate_set.add(candidate_name)

        candidate_list = list(candidate_set)
        assert len(candidate_list) <= top2k, f"candidate union exceeds {top2k}: got {len(candidate_list)}"
        ...
        sorted_index_names = [candidate_name for candidate_name, _ in sorted_candidates][:topk]
        sorted_index_names_list.append(sorted_index_names)

        # for analysis / debug file
        t2i_cand_sorted = [candidate_name for candidate_name, _ in sorted(
            confidence_map1.items(), key=lambda x: x[1], reverse=True
        )][:topk]
        i2i_cand_sorted = [candidate_name for candidate_name, _ in sorted(
            confidence_map2.items(), key=lambda x: x[1], reverse=True
        )][:topk]

        recall_result = {
            "query_id": str(query_ids[i]) if i < len(query_ids) else str(i),
            "image_index": _to_canonical_name_from_gt(reference_names[i]) if i < len(reference_names) else "",
            "target_name": _to_canonical_name_from_gt(target_names[i]) if split != 'test' and i < len(target_names) else "",
    ...
    # --------------------------------------------------
    # test split: same style as stage1 `circo`
    # --------------------------------------------------
    if split == 'test':
        print('Generating test submission file!')

        queryid_to_retrieved_images = {
            query_id: query_sorted_names[:50]
            for (query_id, query_sorted_names) in zip(query_ids, sorted_index_names_list)
        }
        ...
        if _should_save_outputs(kwargs):
            ...
            output_path = (
                f"{dataset_path}/task/{task}/"
                f"test_submissions_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_{ways}_loop_{loop}_{get_time()}.json"
            )
            with open(output_path, 'w') as f:
                json.dump(queryid_to_retrieved_images, f, indent=6)
            ...
            top_rank_path = (
                f"{dataset_path}/task/{task}/"
                f"top_rank_fused_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_{ways}_loop_{loop}_{task}_{get_time()}.json"
            )
            with open(top_rank_path, 'w') as f:
                json.dump(recall_results, f, indent=4)

            print(f"Results saved to: {top_rank_path}")

        return None, sorted_index_names_list

    # --------------------------------------------------
    # val split: same metric computation style as stage1 `circo`
    # --------------------------------------------------
    retrievals = [5, 10, 25, 50]
    recalls = {key: [] for key in retrievals}
    maps = {key: [] for key in retrievals}

    for sorted_index_names, target_name, sub_targets in tqdm.tqdm(
        zip(sorted_index_names_list, target_names, targets),
        total=len(sorted_index_names_list),
        desc='Computing Metric.'
    ):
        target_name = _to_canonical_name_from_gt(target_name)

        sub_targets = np.array([
            _to_canonical_name_from_gt(x)
            for x in np.array(sub_targets)
            if str(x) != ''
        ])
        ...
        for key in retrievals:
            denom = min(len(sub_targets), key)
            if denom == 0:
                maps[key].append(0.0)
            else:
                maps[key].append(float(torch.sum(precisions[:key]) / denom))

        # keep the same single-GT recall definition as stage1
        if len(sub_targets) > 0:
            assert target_name == sub_targets[0], f"Target name not in GTs {target_name} {sub_targets}"

        single_gt_labels = torch.tensor(np.array(sorted_index_names) == target_name)
        for key in retrievals:
            recalls[key].append(float(torch.sum(single_gt_labels[:key])))

    sorted_index_names_list = np.array(sorted_index_names_list, dtype=object)
    ...
    if _should_save_outputs(kwargs):
        ...
        result_path = (
            f'{dataset_path}/task/{task}/'
            f'result_{_clip_prefix_from_kwargs(kwargs)}{dataset_name}_{ways}_loop_{loop}_{get_time()}.json'
        )
        with open(result_path, 'w') as f:
            json.dump(output_metrics, f, indent=6)
